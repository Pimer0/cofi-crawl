...
class QACrawler {
  ...
  findSemanticQuestions($, html, url) {
    ...
    $title.each((index, element) => {
      ...
      if (
        $divParent.attr("itemtype") !== "https://schema.org/Question" ||
        $divParent
      ) {
        questionIndex++;
        ...
        const questionData = {
          ...
          isValid: false,
          issues: [
            "Question détectée en format sémantique - manque les données structurées schema.org",
          ],
          structure: {},
        };
  ...
  validateSchemaQuestionStructure($question, questionData) {
    ...
    const divReponse = $question.children("h2").next();
    ...
    if (itemProp !== "mainEntity") {
      questionData.issues.push(
        `itemprop incorrect sur Question: "${itemProp}" au lieu de "mainEntity"`
      );
      questionData.isValid = false;
    }
    ...
    if (itemType !== "https://schema.org/Question") {
      questionData.issues.push(
        `itemtype incorrect sur Question: "${itemType}"`
      );
      questionData.isValid = false;
    }
    ...
    if ($questionTitle.length === 0) {
      questionData.issues.push(
        'Élément avec itemprop="name" manquant pour le titre de la question'
      );
      questionData.isValid = false;
    } else {
      questionData.structure.questionTitle = $questionTitle.text().trim();
      questionData.structure.questionElement = $questionTitle
        .prop("tagName")
        .toLowerCase();

      // Vérifie que l'élément suivant est une <div>
      const $next = $questionTitle.next();
      if (!$next.is("div")) {
        questionData.issues.push(
          "Aucune <div> immédiatement après le <h2>, donc réponse probablement manquante"
        );
        questionData.isValid = false;
      }
    }
  ...
  validateSchemaAnswerStructure($question, questionData) {
    ...
    if (answerItemProp !== "acceptedAnswer") {
      questionData.issues.push(
        `itemprop incorrect sur Answer: "${answerItemProp}" au lieu de "acceptedAnswer"`
      );
      questionData.isValid = false;
    }
    ...
    if (answerItemType !== "https://schema.org/Answer") {
      questionData.issues.push(
        `itemtype incorrect sur Answer: "${answerItemType}"`
      );
      questionData.isValid = false;
    }
    ...
    if ($answerText.length === 0) {
      questionData.issues.push(
        'Élément avec itemprop="text" manquant pour la réponse'
      );
      questionData.isValid = false;
    } else {
      questionData.structure.answerText =
        $answerText.text().trim().substring(0, 200) + "...";
      questionData.structure.answerElement = $answerText
        .prop("tagName")
        .toLowerCase();
    }
  }
  ...
  validateSemanticQuestionStructure($element, questionData, $) {
    ...
    questionData.issues.push("Attribut itemscope manquant sur Question");
    questionData.issues.push(
      'Attribut itemprop="mainEntity" manquant sur Question'
    );
    questionData.issues.push(
      'Attribut itemtype="https://schema.org/Question" manquant sur Question'
    );
    questionData.issues.push(
      'Élément avec itemprop="name" manquant pour le titre de la question'
    );
    ...
    if (questionText.length < 10) {
      questionData.issues.push(
        "Titre de question trop court (moins de 10 caractères)"
      );
    }
    ...
    if (!$nextContent || $nextContent.length === 0) {
      questionData.issues.push(
        "Aucun contenu de réponse trouvé après la question"
      );
      questionData.issues.push(
        'Élément Answer avec itemtype="https://schema.org/Answer" manquant'
      );
    } else {
      ...
      questionData.issues.push("Attribut itemscope manquant sur Answer");
      questionData.issues.push(
        'Attribut itemprop="acceptedAnswer" manquant sur Answer'
      );
      questionData.issues.push(
        'Élément avec itemprop="text" manquant pour la réponse'
      );
      ...
      if (answerText.length < 20) {
        questionData.issues.push(
          "Contenu de réponse trop court (moins de 20 caractères)"
        );
      }
    }
  }
  ...
  findAnswerContent($questionElement, $) {
    ...
    while ($current.length > 0) {
      ...
      if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName)) {
        ...
        const questionLevel = parseInt(
          $questionElement.prop("tagName").substring(1)
        );
    ...
    for (const url of urls) {
      ...
      if (result.notValidQuestions > 0) {
        koUrls.push({
          ...
          issues: result.questions
            .filter((q) => !q.isValid)
            .flatMap((q) => q.issues),
        });
        console.log(
          `❌ URL avec erreurs: ${url} (${result.notValidQuestions}/${result.totalQuestions} questions avec erreurs)`
        );
      } else {
        console.log(
          `✅ URL OK: ${url} (${result.validQuestions} questions valides)`
        );
      }
  ...
  async generateReport(results, urls = null) {
    const report = {
      summary: {
        ...
        urlsWithErrors: results.filter((r) => (r.notValidQuestions || 0) > 0)
          .length,
        totalQuestions: results.reduce(
          (sum, r) => sum + (r.totalQuestions || 0),
          0
        ),
        totalValidQuestions: results.reduce(
          (sum, r) => sum + (r.validQuestions || 0),
          0
        ),
        totalInvalidQuestions: results.reduce(
          (sum, r) => sum + (r.notValidQuestions || 0),
          0
        ),
      },
      koUrls: [],
    ...
    console.log(
      `Questions avec erreurs: ${report.summary.totalInvalidQuestions}`
    );

    if (report.summary.totalQuestions > 0) {
      console.log(
        `Taux de validité: ${(
          (report.summary.totalValidQuestions / report.summary.totalQuestions) *
          100
        ).toFixed(2)}%`
      );
    }
    ...
    if (report.koUrls.length > 0) {
      ...
      report.koUrls.forEach((koUrl) => {
        console.log(
          `❌ ${koUrl.url} - ${koUrl.notValidQuestions}/${koUrl.totalQuestions} questions avec erreurs`
        );
      });
    }
